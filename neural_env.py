...
class NeuralEnv:
    ...
    class Network:
        ...
        def generate_neuron(self):
            if len(self.get_neuron_list(0,
                                        self.nenv.layer_amt)) == self.nenv.input_neuron_amt + self.nenv.max_h_neuron_amt * self.nenv.max_hidden_layer_amt + self.nenv.output_neuron_amt:
                return
            ...
            for network in self.networks:
                ...
                for i in range(0, len(self.inputs)):
                    ...
                    for j in range(0, self.output_neuron_amt):
                        network.fitness += abs(
                            self.desired_outputs[i][j] - network.neuron[self.output_layer][j].value) ** self.cost_degree
        ...
        for i in range(1, self.layer_amt):
            for j in range(0, len(parent.neuron[i])):
                ...
                for weight in parent.neuron[i][j].weights:
                    ...
                    child.add_weight(child.neuron[i][j], child.neuron[weight.w_neuron.layer][weight.w_neuron.id],
                                     weight_value)
    ...
    def evaluate_fitness_backpropagation(self):
        if len(self.inputs) == 0:
            ...
            for i in range(0, len(self.inputs)):
                ...
                for j in range(0, self.output_neuron_amt):
                    self.network.fitness += (self.desired_outputs[i][j] - self.network.neuron[self.output_layer][
                        j].value) ** 2
            ...
            for i in range(len(self.inputs)):
                ...
                for neuron in self.network.neuron[j]:
                    ...
                    for weight in neuron.weights:
                        ...
                        a_nudge = self.max_aim_nudge_amt * (weight.value / self.weight_cap) * abs(
                            cost)  # TODO doesn't work if cap and floor are different
        ...
        for i in range(self.layer_amt):
            for neur_dict in net_dict['neuron'][i]:
                ...
                for weight_dict in neur_dict['weights']:
                    weight = self.Weight(network.neuron[weight_dict['w_neuron'][0]][weight_dict['w_neuron'][1]],
                                         weight_dict['value'])
